...
function RichstarlineResultHistory() {
  ...
  const getdata = async () => {
    try {
      ...
      const res = await FOR_POST_REQUEST(`history/starlineHistory`, data);
      if (res) {
        setdata(res.data);
    ...
    <div>
      <div className="game-result-card rounded-pill mb-2 d-flex">
        ...
        <div className="col-6 p-2 game-result-right">
          <span className="w-50">Starline Result</span>
        </div>
      </div>
      {data.map((item, i) => (
        <div key={item.providerId} className="py-3 px-3 my-2 border rounded">
          <div className="d-flex primary-color fw-bold justify-content-between align-items-center  ">
            ...
            <span>
              {item.winningDigit}-{item.winningDigitFamily}
            </span>
          </div>
        </div>
      ))}
...
export default RichstarlineResultHistory;
